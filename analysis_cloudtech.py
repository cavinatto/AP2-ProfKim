...
# 2. Certifique-se de que o arquivo 'latencia_sistemas_cloud.csv' está no caminho correto (ex: /content/drive/My Drive/)
# 3. Execute o script
...
def main():
    ...
    try:
        ...
        # Assumir que o arquivo está em /content/drive/My Drive/
        global DATA_FILE
        DATA_FILE = '/content/drive/My Drive/latencia_sistemas_cloud.csv'
        print(f"Arquivo definido como: {DATA_FILE}")
        print("Se o arquivo estiver em outro local, edite o caminho acima.")
    except ImportError:
        pass  # Não está no Colab, usa o caminho local
    ...
    plt.hist(lat, bins=30, edgecolor='black', alpha=0.7)
    ...
    plt.boxplot(lat, vert=False, patch_artist=True, boxprops=dict(facecolor='#a0cbe8'), medianprops=dict(color='black'))
    plt.title('Boxplot de Latência (ms)')
    plt.xlabel('latency_ms')
    plt.show()
    ...
    lines.append('- Os gráficos são exibidos acima.')
    ...
    lines.append('Use o boxplot exibido acima para evidenciar a cauda longa e outliers de latência, e a concentração central da maioria das medições.')
    ...
    # Para Google Colab: exibir o relatório
